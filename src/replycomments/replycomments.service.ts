...
export class ReplycommentsService {
    ...
    async updateReplyComment(replycommentUpdateRequestDto: ReplyCommentUpdateRequestDto) {
      const replycomment = await this.replycommentsRepository
        ...
        .leftJoin('r.Users', 'u')
        .where('r.replyIdx = :replyIdx', { replyIdx: replycommentUpdateRequestDto.replyIdx })
        .andWhere('u.userIdx = :userIdx', { userIdx: replycommentUpdateRequestDto.userIdx })
        .getOne();
      ...
      await this.replycommentsRepository.update(replycommentUpdateRequestDto.commentIdx, {
    ...
    async deleteReplyComment(replycommentDeleteRequestDto: ReplyCommentDeleteRequestDto) {
      const replycomment = await this.replycommentsRepository
        .createQueryBuilder('r')
        .select(['r.replyIdx', 'u.userIdx'])
        .leftJoin('r.Users', 'u')
        .where('r.replyIdx = :replyIdx', { replyIdx: replycommentDeleteRequestDto.replyIdx })
        .andWhere('u.userIdx = :userIdx', { userIdx: replycommentDeleteRequestDto.userIdx })
        .getOne();
    ...
    async getAllReplyComment(replycommentGetRequestDto: ReplyCommentGetRequestDto  ) {

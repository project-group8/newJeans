...
export class Migrations1682523574874 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`Tokens\` (\`tokenIdx\` varchar(36) NOT NULL, \`userIdx\` varchar(255) NOT NULL, \`token\` text NOT NULL, \`createdAt\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deletedAt\` timestamp(6) NULL, UNIQUE INDEX \`REL_5e2389ba2f7642aed3dc0df4b2\` (\`userIdx\`), PRIMARY KEY (\`tokenIdx\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`Users\` (\`userIdx\` varchar(36) NOT NULL, \`email\` varchar(255) NOT NULL, \`password\` varchar(255) NULL, \`nickname\` varchar(255) NOT NULL, \`provider\` varchar(6) NULL, \`level\` varchar(255) NOT NULL DEFAULT '훈수 초보', \`status\` varchar(255) NOT NULL DEFAULT 'US000001', \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deletedAt\` timestamp(6) NULL, UNIQUE INDEX \`IDX_5da3f86a40ce07289424c734c9\` (\`nickname\`), PRIMARY KEY (\`userIdx\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`ChatSaves\` (\`chatSaveIdx\` int NOT NULL AUTO_INCREMENT, \`nickname\` varchar(255) NOT NULL, \`room\` varchar(255) NOT NULL, \`saveData\` varchar(255) NOT NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), PRIMARY KEY (\`chatSaveIdx\`)) ENGINE=InnoDB`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`DROP INDEX \`REL_5e2389ba2f7642aed3dc0df4b2\` ON \`Tokens\``);
